...
MEMBRANE_CURVATURE_DATA = {
    ...
    'z_ref': {
        'small': np.array([[10.0, 11.0, 12.0], [15.0, 20, 25], [15.0, 40, 50]]),
        'all': np.array(
            [
                [
                    15.96339989,
    ...
    'gaussian_curvature': {
        'small': np.array(
            [
                [-2.19478738e-02, -2.32254318e-03, -5.47176909e-04],
    ...
    'mean_curvature': {
        'small': np.array(
            [
                [-0.16037507, -0.04033506, -0.02057139],
    ...
    'grid': {
        'small': {
            'upper': np.array([[15.0, 15.0, np.nan], [15.0, np.nan, np.nan], [15.0, np.nan, np.nan]]),
            'lower': np.array([[np.nan, np.nan, 12.0], [np.nan, 12.0, 12.0], [np.nan, 12.0, 12.0]]),
        }
    },
    'beads': {'small': {'upper': {'POPC': [0, 1, 2, 3]}, 'lower': {'POPC': [4, 5, 6, 7, 8]}}},
}
...
def parametric_hemisphere(radius: float, nx: int, ny: int, scale: float = 5) -> tuple[np.ndarray, float, float]:
    ...
    for radius in np.arange(5, 100, 5):
        for nx in [7, 23, 47, 67]:
            for ny in [7, 23, 47, 67]:
                ...
                height_field, H, K, dx, dy = parametric_hemisphere(radius=radius, nx=nx, ny=ny, scale=200)
...
def parametric_saddle(length: float, nx: int, ny: int, scale: float = 1000) -> tuple[np.ndarray, float, float]:
    ...
    H = 27 * xx * (-(xx**4) + 2 * xx**2 * yy**2 + 3 * yy**4) / (1 + 9 * (xx**2 + yy**2) ** 2) ** (3 / 2)
    return zz, K, H, dx, dy
...
def test_parametric_saddle_curvature():
    for nx in [7, 23, 47, 67]:
        for ny in [7, 23, 47, 67]:
            height_field, K, H, dx, dy = parametric_saddle(length=10, nx=nx, ny=ny, scale=800)
            mean = mean_curvature(height_field, dx, dy)
            gauss = gaussian_curvature(height_field, dx, dy)
...
@pytest.mark.parametrize(
    'n_cells, grid_z_coords',
    [
        (3, np.full((3, 3), 10.0)),
        (3, np.array([[10.0, 20.0, 30.0], [10.0, 20.0, 30.0], [10.0, 20.0, 30.0]], dtype=float)),
    ],
)
...
def test_normalized_grid_more_beads():
    ...
    grid_z_coords = np.full((3, 3), 10.0)
    # grid number of beads per unit
    norm_grid = np.array([[2.0, 1.0, 1.0], [1.0, 2.0, 1.0], [1.0, 1.0, 2.0]])
    # avg z coordinate in grid
    expected_normalized_surface = np.array([[5.0, 10.0, 10.0], [10.0, 5.0, 10.0], [10.0, 10.0, 5.0]])
...
def test_derive_surface(small_grofile):
    ...
    expected_surface = np.array(([150.0, 150.0, 120.0], [150.0, 120.0, 120.0], [150.0, 120.0, 120.0]))
...
@pytest.mark.parametrize(
    ...
    [
        (3, 3, (0, 300), (0, 300), np.array(([150.0, np.nan, 150.0], [np.nan, 150.0, 150.0], [150.0, 150.0, 150.0]))),
        (
            3,
            4,
            (0, 300),
            (0, 400),
            np.array([[150.0, np.nan, 150.0, np.nan], [np.nan, 150.0, 150.0, np.nan], [150.0, 150.0, 150.0, np.nan]]),
        ),
    ],
)
...
class TestMembraneCurvature(object):
    ...
    def dummy_surface(self):
        surface = np.array([[110.0, 150.0, 110.0], [150.0, 150.0, 150.0], [150.0, 150.0, 150.0]])
        return surface

    @pytest.fixture()
    def curvature_unwrapped_universe(self, universe_dummy_wrap):
        return MembraneCurvature(
            universe_dummy_wrap, surface_method='binning', fft_filter=None, n_x_bins=3, n_y_bins=3
        ).run()
    ...
    def test_wrap_defaults_true_binning(self, universe):
        mc = MembraneCurvature(universe, select='name PO4', surface_method='binning', fft_filter=None)
    ...
    def test_grid_bigger_than_simulation_box_x_dim(self, universe):
        regex = (
            r'Grid range in x does not cover entire '
            r'dimensions of simulation box.\n Minimum dimensions '
            r'must be equal to simulation box.'
        )
        with pytest.warns(UserWarning, match=regex):
            MembraneCurvature(universe, select='name PO4', surface_method='binning', fft_filter=None, x_range=(0, 10))

    def test_grid_bigger_than_simulation_box_y_dim(self, universe):
        regex = (
            r'Grid range in y does not cover entire '
            r'dimensions of simulation box.\n Minimum dimensions '
            r'must be equal to simulation box.'
        )
        with pytest.warns(UserWarning, match=regex):
            MembraneCurvature(universe, surface_method='binning', fft_filter=None, select='name PO4', y_range=(0, 10))
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                (0, 300),
                (0, 300),
                np.array(([150.0, np.nan, 150.0], [np.nan, 150.0, 150.0], [150.0, 150.0, 150.0])),
            ),
            (
                3,
                4,
                ...
                (0, 400),
                np.array(
                    [[150.0, np.nan, 150.0, np.nan], [np.nan, 150.0, 150.0, np.nan], [150.0, 150.0, 150.0, np.nan]]
                ),
            ),
        ],
    )
    ...
    def test_analysis_get_z_surface_wrap_xy(self, universe_dummy_wrap_xy, dummy_surface):
        x_bin = y_bin = 3
        mc = MembraneCurvature(
            universe_dummy_wrap_xy, surface_method='binning', fft_filter=None, n_x_bins=x_bin, n_y_bins=y_bin
        ).run()
        avg_surface = mc.results.average_z_surface
        assert_almost_equal(avg_surface, dummy_surface)
    ...
    def test_analysis_gaussian_wrap(self, curvature_unwrapped_universe, dummy_surface):
        ...
        expected_gaussian = gaussian_curvature(
            dummy_surface, curvature_unwrapped_universe.dx, curvature_unwrapped_universe.dy
        )
    ...
    def test_analysis_get_z_surface_no_wrap(self, universe_dummy_wrap):
        ...
        x_bin = y_bin = 3
        mc = MembraneCurvature(
            universe_dummy_wrap, surface_method='binning', fft_filter=None, n_x_bins=x_bin, n_y_bins=y_bin, wrap=False
        ).run()
        avg_surface = mc.results.average_z_surface
        assert_almost_equal(avg_surface, expected_surface)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (3, 3, np.array([[150.0, 120.0, 150.0], [150.0, 120.0, 120.0], [150.0, 120.0, 120.0]])),
            (
                4,
                4,
                np.array(
                    [
                        [150.0, 120.0, 150.0, np.nan],
            ...
            (
                ...
                np.array(
                    [
                        [150.0, 120.0, np.nan, 150.0, np.nan],
    ...
    def test_analysis_get_z_surface_dummy_full(self, universe_dummy_full, x_bin, y_bin, expected_surface):
        mc = MembraneCurvature(
            universe_dummy_full, surface_method='binning', fft_filter=None, n_x_bins=x_bin, n_y_bins=y_bin
        ).run()
        avg_surface = mc.results.average_z_surface
        assert_almost_equal(avg_surface, expected_surface)
    ...
    def test_analysis_mean(self, universe_dummy_full):
        expected_mean = np.array(
            [
                [0.001318109566818, 0.0015, 0.001702672969121],
                [0.000659054783409, 0.000725381187923, 0.001467083452574],
                [0.000659054783409, 0.000725381187923, 0.0015],
            ]
        )
        mc = MembraneCurvature(
            universe_dummy_full, surface_method='binning', fft_filter=None, n_x_bins=3, n_y_bins=3
        ).run()
        avg_mean = mc.results.average_mean
        assert_almost_equal(avg_mean, expected_mean)
    ...
    def test_test_analysis_no_wrapping(self, universe):
        ...
        with pytest.warns(UserWarning, match=regex):
            MembraneCurvature(universe, surface_method='binning', fft_filter=None, wrap=False)

    @pytest.mark.parametrize(
        'x_bin, y_bin, box_dim, dummy_array',
        [
            # test too large x coordinates 2 bins
            (2, 2, 200, np.array([[0.0, 0.0, 150.0], [200.0, 0.0, 150.0], [0.0, 100.0, 150.0], [100.0, 100.0, 120.0]])),
            # test too large y coordinates 2 bins
            (2, 2, 200, np.array([[0.0, 0.0, 150.0], [100.0, 0.0, 150.0], [0.0, 200.0, 150.0], [100.0, 100.0, 150.0]])),
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                2,
                200,
                np.array([[-150.0, 0.0, 150.0], [100.0, 0.0, 150.0], [0.0, 100.0, 150.0], [100.0, 100.0, 150.0]]),
            ),
            ...
            (
                ...
                2,
                200,
                np.array([[0.0, 0.0, 150.0], [200.0, 0.0, 150.0], [0.0, -150.0, 150.0], [100.0, 100.0, 120.0]]),
            ),
